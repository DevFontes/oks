#cloud-config
write_files:
  - path: /etc/systemd/system/k3s-firewall.service
    permissions: '0644'
    content: |
      [Unit]
      Description=Configure firewall for K3s
      After=firewalld.service
      Wants=network-online.target
      After=network-online.target

      [Service]
      Type=oneshot
      RemainAfterExit=yes
      ExecStart=/usr/bin/firewall-cmd --permanent --add-port=6443/tcp
      ExecStart=/usr/bin/firewall-cmd --permanent --add-port=2379/tcp
      ExecStart=/usr/bin/firewall-cmd --permanent --add-port=2380/tcp
      ExecStart=/usr/bin/firewall-cmd --reload

      [Install]
      WantedBy=multi-user.target

runcmd:
  - swapoff -a
  - sed -i.bak '/ swap / s/^/#/' /etc/fstab
  - until curl -skf https://${server_ip}:6443/ping; do sleep 5; done
  - curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="server --server https://${server_ip}:6443 --token ${k3s_token} --tls-san ${k3s_api_dns}.${domain} --disable traefik --disable metrics-server --disable helm-controller --disable servicelb --flannel-backend=host-gw" sh - | tee /var/log/k3s-join-install.log